...
class SessionManager:
    ...
    def __init__(self, sessions_config: DictConfig) -> None:
        ...
        required_keys = ["SESSION_ID_COLUMN", "INCOMING_SESSIONS_CSV_PATH", 
                        "SESSIONS_TO_SKIP_CSV_PATH", "REPORTED_SESSIONS_CSV_PATH",
                        "SESSION_TYPES"]
        missing_keys = [key for key in required_keys if key not in sessions_config]
        assert not missing_keys, f"Required keys missing from sessions_config: {missing_keys}"
        ...
        self.incoming_sessions_df = self._get_new_sessions(
            self.incoming_sessions_df, 
            self.sessions_to_skip_df
        )
    ...
    def _load_sessions_to_skip(self) -> pl.DataFrame:
        ...
        return (pl.read_csv(skip_path) if os.path.exists(skip_path) 
                else pl.DataFrame())

    def _load_reported_sessions(self) -> pl.DataFrame:
        """Load previously reported sessions."""
        reported_path = self.sessions_config["REPORTED_SESSIONS_CSV_PATH"] 
        return (pl.read_csv(reported_path) if os.path.exists(reported_path)
                else pl.DataFrame())

    def _get_new_sessions(self, df_to_filter: pl.DataFrame, 
                         df_to_filter_out: pl.DataFrame) -> pl.DataFrame:
    ...
    def _save_reported_sessions(self) -> None:
        ...
        self.reported_sessions_df.write_csv(self.sessions_config["REPORTED_SESSIONS_CSV_PATH"])

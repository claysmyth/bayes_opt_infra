...
def default_ax(config: dict) -> AxClient:
    ...
    ax_client.create_experiment(
        ...
        choose_generation_strategy_kwargs={
            "num_initialization_trials": config['experiment'].get('num_initialization_trials', 5)
        }
    )
...
def main():
    ...
    if config['experiment'].get('save_json', True):
        json_path = Path(config['paths']['base_dir']) / f"{config['experiment']['name']}_initial.json"

...
# NOTE: CURRENLTY ONLY WORKS FOR A SINGLE TRIAL. ALSO, YOU MAY NEED TO TEMPORARILY ADJUST THE TARGET AMPS CACHE FILE TO CORRECTLY EVAULATE THE REWARD FUNCTION.


"""
Usage:

python dags/attach_trials_to_experiment.py 

This script is used to attach completed trials to an existing experiment, on which the current running trial is a different set of suggested parameters.

It is intended to use the same exact setup as the main pipeline (i.e. the same hydra configurations), except is ingests sessions and parameters from a json file (see SESSIONS_TO_PROCESS_PATH).

If using with RC+S, you will also need to adjust the target amps cache (current_target_amps.json) file to to contain the parameters of the trial that is being attached, so that the reward function can be evaluated.

json format:

{
    "sessions_list": ["session_id_1", "session_id_2"],
    "trial_parameters": {"parameter_1": value_1, "parameter_2": value_2}
}

"""

SESSIONS_TO_PROCESS_PATH = "/home/claysmyth/code/bayes_opt_infra/configs/attach_trials_to_experiment_config/attach_trials.json"

def attach_trials_to_experiment(cfg: DictConfig, sessions_list, trial_parameters):
    ...
    # Get sessions to be attached to experiment
    sessions_info = session_manager.incoming_sessions_df.filter(
        pl.col(session_manager.session_id_column).is_in(sessions_list)
    )
    ...
    for participant_sessions in sessions_info.partition_by(participant_column):
        ...
        trial_index = experiment_tracker.attach_trial(trial_parameters)
        ...
        # Complete the trial
        experiment_tracker.complete_trial(trial_index, result)
...
def hydra_attach_trials(cfg: DictConfig):
    with open(SESSIONS_TO_PROCESS_PATH, 'r') as f:
        sessions_to_process = json.load(f)
    sessions_list = sessions_to_process.get('sessions_list', [])
    trial_parameters = sessions_to_process.get('trial_parameters', {})
    attach_trials_to_experiment(cfg, sessions_list, trial_parameters)
